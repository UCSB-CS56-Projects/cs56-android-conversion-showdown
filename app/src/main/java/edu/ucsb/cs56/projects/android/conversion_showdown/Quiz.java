...
public class Quiz extends Activity implements OnClickListener {
    ...
    /**
     * The variable to track index of the ArrayList with key values
     * KEY
    ...
    /**
     * Computing the result and compring the actual result with the one user provides to check correctness,
     * and then informing the user of the correct answer if user provides an incorrect one. Also keeping track of
     * the progress and ending the test activity according to the chosen test length.
     * @param view
     */
    public void TestResult(View view) {
        MyUtils.hideSoftKeyBoard(view, (InputMethodManager) getSystemService(INPUT_METHOD_SERVICE));
        //Setting the radix based on key, user entry determines the radix number
        ...
        Long userAnswer;
        userAnswer = Long.parseLong(sAnswer, radix);
